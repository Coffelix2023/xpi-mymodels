...
const DATA=JSON.parse(${JSON.stringify(data)});let lang="zh",zoom=1,providerIndex=0,modelIndex=0,mode="edit",busy=false,draft=null;const text={zh:{title:"Pi 模型配置",lang:"EN",langTitle:"切换语言",provider:"Provider",model:"Model",providerSection:"Provider",modelSection:"Model",providerName:"显示名称",providerId:"Provider ID",api:"API 类型",baseUrl:"Base URL",apiKey:"API Key",apiKeyPlaceholder:"留空以保留当前值",authHeader:"自动发送 Authorization",modelName:"模型名称",modelId:"模型 ID",modelApi:"模型 API 覆盖",modelBaseUrl:"模型 Base URL 覆盖",contextWindow:"上下文窗口",maxTokens:"最大输出 tokens",reasoning:"支持 reasoning",imageInput:"支持图片输入",costInput:"输入价格 / 1M tokens",costOutput:"输出价格 / 1M tokens",costCacheRead:"缓存读取价格 / 1M tokens",costCacheWrite:"缓存写入价格 / 1M tokens",add:"添加模型",delete:"删除模型",cancel:"取消",save:"保存",empty:"没有可用的 Provider",emptyModels:"当前 Provider 没有模型",deleteConfirm:"确认删除当前模型？",deleteProviderConfirm:"确认删除当前 Provider 及其全部模型？",addProviderPrompt:"新 Provider ID",addProviderTitle:"新增 Provider",deleteProviderTitle:"删除 Provider",added:"新模型草稿",saved:"已保存"},en:{title:"Pi model configuration",lang:"中文",langTitle:"Switch language",provider:"Provider",model:"Model",providerSection:"Provider",modelSection:"Model",providerName:"Display name",providerId:"Provider ID",api:"API type",baseUrl:"Base URL",apiKey:"API key",apiKeyPlaceholder:"Leave blank to keep current value",authHeader:"Send Authorization automatically",modelName:"Model name",modelId:"Model ID",modelApi:"Model API override",modelBaseUrl:"Model Base URL override",contextWindow:"Context window",maxTokens:"Max output tokens",reasoning:"Supports reasoning",imageInput:"Supports image input",costInput:"Input price / 1M tokens",costOutput:"Output price / 1M tokens",costCacheRead:"Cache read price / 1M tokens",costCacheWrite:"Cache write price / 1M tokens",add:"Add model",delete:"Delete model",cancel:"Cancel",save:"Save",empty:"No providers available",emptyModels:"This provider has no models",deleteConfirm:"Delete the current model?",deleteProviderConfirm:"Delete this provider and all of its models?",addProviderPrompt:"New provider ID",addProviderTitle:"Add provider",deleteProviderTitle:"Delete provider",added:"New model draft",saved:"Saved"}};const el=id=>document.getElementById(id),t=key=>text[lang][key]??key;function applyText(){document.querySelectorAll("[data-i18n]").forEach(node=>node.textContent=t(node.dataset.i18n));document.querySelectorAll("[data-i18n-placeholder]").forEach(node=>node.placeholder=t(node.dataset.i18nPlaceholder));el("lang").title=t("langTitle");el("addProvider").title=t("addProviderTitle");el("deleteProvider").title=t("deleteProviderTitle")}function setPanelStatus(result){busy=false;render();const status=el("status");status.textContent=result.message;status.className="status"+(result.ok?" success":" error")}function applyProviders(payload){DATA.providers=payload.providers;providerIndex=Math.max(0,Math.min(payload.providerIndex??0,Math.max(0,DATA.providers.length-1)));modelIndex=0;mode="edit";draft=null;busy=false;render()}function selectOptions(select,values,empty){select.textContent="";if(empty){const first=document.createElement("option");first.value="";first.textContent=lang==="zh"?"使用 Provider 默认":"Use provider default";select.appendChild(first)}values.forEach(value=>{const option=document.createElement("option");option.value=value;option.textContent=value;select.appendChild(option)})}function blankModel(){return{id:"",name:"",api:"",baseUrl:"",contextWindow:128000,maxTokens:16384,reasoning:false,imageInput:false,cost:{input:0,output:0,cacheRead:0,cacheWrite:0}}}function currentModel(){return mode==="add"?draft:DATA.providers[providerIndex]?.models[modelIndex]}function renderTabs(){const tabs=el("tabs");tabs.textContent="";DATA.providers.forEach((item,index)=>{const button=document.createElement("button");button.type="button";button.className="tab"+(index===providerIndex?" active":"");button.textContent=item.name||item.id;button.title=item.id;button.addEventListener("click",()=>{providerIndex=index;modelIndex=0;mode="edit";draft=null;render()});tabs.appendChild(button)})}function render(){const provider=DATA.providers[providerIndex];renderTabs();selectOptions(el("model"),provider?provider.models.map(item=>item.name+" ("+item.id+")"):[],false);el("model").selectedIndex=mode==="add"?-1:modelIndex;const model=currentModel();const hasProvider=Boolean(provider);const hasModel=Boolean(model);["providerName","api","baseUrl"].forEach(id=>el(id).disabled=!hasProvider||busy);["modelName","modelId","modelApi","modelBaseUrl","contextWindow","maxTokens","costInput","costOutput","costCacheRead","costCacheWrite"].forEach(id=>el(id).disabled=!hasModel||busy);el("save").disabled=!hasModel||busy;el("delete").disabled=!hasModel||mode==="add"||busy;el("add").disabled=!hasProvider||busy;el("addProvider").disabled=busy;el("deleteProvider").disabled=!hasProvider||busy;if(!hasProvider){el("status").textContent=t("empty");el("status").className="status";return}el("providerName").value=provider.name;el("providerId").value=provider.id;selectOptions(el("api"),DATA.apiOptions,false);el("api").value=provider.api;el("baseUrl").value=provider.baseUrl;el("apiKey").value="";el("authHeader").checked=provider.authHeader;if(!hasModel){el("status").textContent=mode==="add"?t("added"):t("emptyModels");el("status").className="status";return}el("modelName").value=model.name;el("modelId").value=model.id;selectOptions(el("modelApi"),DATA.apiOptions,true);el("modelApi").value=model.api;el("modelBaseUrl").value=model.baseUrl;el("contextWindow").value=String(model.contextWindow);el("maxTokens").value=String(model.maxTokens);el("reasoning").checked=model.reasoning;el("imageInput").checked=model.imageInput;el("costInput").value=String(model.cost.input);el("costOutput").value=String(model.cost.output);el("costCacheRead").value=String(model.cost.cacheRead);el("costCacheWrite").value=String(model.cost.cacheWrite);if(mode==="add")el("status").textContent=t("added")}function collect(){return{api:el("api").value,apiKey:el("apiKey").value,authHeader:el("authHeader").checked,baseUrl:el("baseUrl").value,contextWindow:Number(el("contextWindow").value),costCacheRead:Number(el("costCacheRead").value),costCacheWrite:Number(el("costCacheWrite").value),costInput:Number(el("costInput").value),costOutput:Number(el("costOutput").value),imageInput:el("imageInput").checked,maxTokens:Number(el("maxTokens").value),modelApi:el("modelApi").value,modelBaseUrl:el("modelBaseUrl").value,modelId:el("modelId").value,modelName:el("modelName").value,name:el("providerName").value,reasoning:el("reasoning").checked}}function lock(){busy=true;el("save").disabled=true;el("add").disabled=true;el("delete").disabled=true;el("addProvider").disabled=true;el("deleteProvider").disabled=true}function send(action){if(busy)return;lock();window.glimpse.send({action,mode,providerIndex,modelIndex,values:collect()})}el("model").addEventListener("change",event=>{modelIndex=event.target.selectedIndex;mode="edit";draft=null;render()});el("add").addEventListener("click",()=>{if(!DATA.providers[providerIndex])return;mode="add";draft=blankModel();render()});el("delete").addEventListener("click",()=>{if(mode!=="add"&&window.confirm(t("deleteConfirm")))send("delete")});el("addProvider").addEventListener("click",()=>{if(busy)return;const id=window.prompt(t("addProviderPrompt"));if(!id||!id.trim())return;lock();window.glimpse.send({action:"add-provider",id:id.trim()})});el("deleteProvider").addEventListener("click",()=>{if(busy||!DATA.providers[providerIndex])return;if(!window.confirm(t("deleteProviderConfirm")))return;lock();window.glimpse.send({action:"delete-provider",providerIndex})});el("cancel").addEventListener("click",()=>window.glimpse.send({action:"cancel"}));el("form").addEventListener("submit",event=>{event.preventDefault();if(el("form").reportValidity())send("save")});el("lang").addEventListener("click",()=>{lang=lang==="zh"?"en":"zh";applyText();render()});el("zoomOut").addEventListener("click",()=>{zoom=Math.max(.8,zoom-.1);document.body.style.zoom=zoom});el("zoomIn").addEventListener("click",()=>{zoom=Math.min(1.5,zoom+.1);document.body.style.zoom=zoom});el("zoomReset").addEventListener("click",()=>{zoom=1;document.body.style.zoom=zoom});document.addEventListener("keydown",event=>{if(event.key==="Escape")window.glimpse.send({action:"cancel"})});applyText();render();
</script></body></html>`;
}
